...
import com.kp.monitor.presenter.SeparateListPresenter;
import com.kp.monitor.ui.adapter.SeparateListAdapter;
...
public class SeparateActivity extends BaseListActivity<SeparateListModel, SeparateListPresenter, SeparateVO> implements SeparateListView {
    ...
    private int handleStatue = -1;  // 当前状态

    public static void startAction(Context context, String exceptionId) {

        Intent intent = new Intent(context, SeparateActivity.class);
    ...
    protected CommonRecycleViewAdapter getAdapter() {
        return new SeparateListAdapter(mContext, R.layout.list_item_off_line);
    }

    @Override
    public void onSeparateBaicInfoComplete(UnusalInfoVO unusalInfoVO) {
    ...
    public void onHandleComplete(MsgVo msgVo) {
        ...
        if (msgVo.msg.equals("X")) {  //  处理中
            ...
            txtHandleStatue.setText("处理状态: 处理中");
            goHandle.setVisibility(View.VISIBLE);
            goHandle.setText("处理完成");
            ...
            txtHandleStatue.setText("处理状态: 处理完成");
            goHandle.setVisibility(View.GONE);
            handleStatue = UnusalInfoVO.DEAL_STATUS_ED;   // 更新 handleStatue 的状态值
