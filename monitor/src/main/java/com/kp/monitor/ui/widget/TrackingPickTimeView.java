...
public class TrackingPickTimeView extends LinearLayout implements NumberPicker
     ...
     * 视图控件
     */
    private TrackingNumberPicker mNpLeft, mNpMiddle, mNpRight;
    ...
    public TrackingPickTimeView(Context context) {
        super(context);
    ...
    public TrackingPickTimeView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        generateView();
        initPicker();
    ...
    public TrackingPickTimeView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void generateView() {
        ...
        container.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup
                .LayoutParams.WRAP_CONTENT));
        container.setGravity(Gravity.CENTER);
        mNpLeft = new TrackingNumberPicker(mContext);
        mNpMiddle = new TrackingNumberPicker(mContext);
        mNpRight = new TrackingNumberPicker(mContext);
        mMinuteTip = new TextView(mContext);//中间文字
//        mMinuteTip.setText(":");
        //设置获取焦点 不被子view取代
    ...
    private void updateRightValue(long timeMillis) {
        ...
        if (mCurrentType == TYPE_PICK_DATE) {
            ...
            int days = getMaxDayOfMonth(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH)
                    + 1);
    ...
    public interface onSelectedChangeListener {
        void onSelected(TrackingPickTimeView view, long timeMillis);
    }
}
